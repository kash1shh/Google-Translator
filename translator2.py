...
def data(event=None):
    ...
    if msg:  # Only translate if there is text to translate
...
Source_txt.bind("<KeyRelease>", data)  # Trigger translation on key release

list_text = list(LANGUAGES.values())

comb_sor = ttk.Combobox(root, value=list_text, font=("Times New Roman", 12))
comb_sor.place(x=50, y=230, height=40, width=150)
comb_sor.set("English")
comb_sor.bind("<<ComboboxSelected>>", data)  # Trigger translation on source language change

comb_dest = ttk.Combobox(root, value=list_text, font=("Times New Roman", 12))
comb_dest.place(x=280, y=230, height=40, width=150)
comb_dest.set("Hindi")
comb_dest.bind("<<ComboboxSelected>>", data)  # Trigger translation on destination language change
